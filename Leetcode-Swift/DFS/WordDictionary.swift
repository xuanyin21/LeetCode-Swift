...
class Trie {
    ...
    func search(word: String) -> Bool {
        return dfs(word: word, index: 0, node: root)
    }
    
    fileprivate func dfs(word: String, index: Int, node: TrieNode) -> Bool {
        if index == word.count {
            return node.isEnd
        }
        ...
        if char != "." {
            ...
            return dfs(word: word, index: index + 1, node: nextNode)
        } else {
            for key in node.children.keys {
                if dfs(word: word, index: index + 1, node: node.children[key]!) {
                    return true
                }
            }
